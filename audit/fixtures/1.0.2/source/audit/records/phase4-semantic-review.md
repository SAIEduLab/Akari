...
## Current product 1.0.2: audio import and audit additions

Final wording review also aligned D09-SAVE-002 with product 1.0.2 and both supported old versions, and made the inherited 1.0.1 metadata paragraph explicitly historical. The stable D09 identity, mandatory validations and immutable checkpoint remain unchanged. The intermediate manual's narrow layout was visually inspected; all 46 examples, 139 internal links and 18 viewport checks passed on the local reviewed product. This documentation correction renews the feature freeze and source binding before the next Push; the final Actions head still requires all ten jobs.

The current change is authorized against public main f3cb85c407625a778198330885e2d4697e155c70. Product appVersion advances to 1.0.2. Language/runtime remain 1.0.0 and both formats remain 3. The sections below this addition retain historical decisions for their original snapshots. Current execution and release acceptance require fresh evidence for the final pushed commit.

Reviewed the complete FLAC, Ogg Opus and ISO BMFF/AAC-LC container readers, their bounds and structural checks, canonicalization and native decoding, import transaction, saved asset validator, file picker, exported runtime MIME allowlist and version compatibility paths. Existing MP3/WAV processing, all 38 resource limits, compiler, scheduler and editor transaction mechanisms are retained. FLAC keeps STREAMINFO and frames; Ogg validates sequence, serial, lacing, CRC, BOS/EOS and granules before replacing bounded vendor/comment packets and recomputing CRC; M4A validates a single AAC-LC track, bounded boxes/descriptors and chunk/sample tables, and blanks metadata in same-sized free boxes to preserve sample offsets. Unsupported codec profiles and containers reject before installation. Actual native decoding is mandatory, not inferred from extensions or MIME.

Each of eight independently encoded positive files is exercised through ordinary file input with deliberately misleading name/MIME, shared Undo/Redo, decoded non-silent channel waveforms, canonical idempotence, metadata removal, execution, downloaded save/reload with unchanged asset digest, and downloaded offline player playback. Negative cases cover ALAC/Vorbis/ADTS, unsupported AAC object types, truncated/oversized container fields and sample offsets, CRC/chained Ogg, rate/duration/size limits, canonical saved metadata and MIME mismatch, failed-import state preservation, both old project versions and original Markdown validation, and exported runtime MIME rejection. The latter must reference the sound in actual source because executable export intentionally prunes unused assets. These assertions are additional obligations, not substitutions for the original audio/asset boundary suites.

The immutable 1.0.1/1.0.0/0.8 archives, expected core/language/editor/GUI/browser sets and 15 duplication/drawing feature cases remain unchanged. Only enumerated current version literals are transformed on the immutable expectation side. Full runtime comparison permits exactly the appVersion literal and expanded audio MIME allowlist; any other runtime change remains a failure. The 1.0.2 quality policy is compared against 1.0.1 with only target-version and feature-freeze fields substituted. Feature freeze is recorded before Push and rejects subsequent drift. Source review hashes preserve binary audio fixtures as exact bytes rather than lossy UTF-8 decoding.

Reviewed the workflow job graph, independent bundle validators and aggregate. All original eight jobs remain, with two mandatory audio jobs on Linux and Windows. Chrome for Testing 140.0.7339.207 is installed into an isolated directory because native AAC support differs from the original pinned Chromium; that original Chromium environment and its startup probe remain mandatory in Actions. The aggregate rejects either missing platform, absent/duplicate/failed/skipped audio IDs, wrong snapshot/browser/version/fixture hash, page errors or network access. All new mutation validators are mandatory in static. Fixed fixture generation records source signal, encoder, commands, exact sizes and hashes.

Found and repaired an existing workflow preflight defect: its positive verify(document) call was indented inside verify, so the valid workflow was never verified at top level. It now executes once at top level. All 17 prior invalid-workflow cases remain and five additional feature/audio mutations are rejected. This strengthens the audit contract without changing acceptance thresholds.

Updated LANGUAGE, README, public entry/manual version labels, sound help and the intermediate guide to state all five formats and supported profiles, size/time/channel/rate limits, metadata handling, browser decoding requirements and old-project compatibility. No new language command, external product dependency, persistence path or private instruction is published. All 16 phases and 28 D09 obligations remain applicable through their existing mappings; the inventory adds audio IDs and both required environments. Source review is complete for these changes; local and Actions results are separate. Required evidence must be regenerated after any input change. Physical-device/native-IME/user-study coverage is not claimed.
...
## SAIEL website link review

The public README developer section and the `index.html` footer now link SAIEL to https://sites.google.com/shimane-edu.org/saiel/%E3%83%9B%E3%83%BC%E3%83%A0. The published homepage was inspected in the browser: it identifies SAIEL, describes its educational materials, and introduces Akari with a live demo. The link labels identify the developer and the destination is the user-supplied homepage. The changes affect only two public guide links; `Akari.html`, LANGUAGE, AUDIT, manuals, audit suites, fixed baselines and offline product execution are unchanged. This section records the source and wording impact review for the renewed input binding. Execution evidence remains tied to the final candidate HEAD and its Actions run.

## Product 1.0.1: component duplication and drawing

The authorized change is relative to public main `f8cb84b5da4c8ba01c0267526ffee6d38eaf7bd4`, on `fix/component-duplicate-paint`. The product version advances to 1.0.1; language/runtime remain 1.0.0 and both formats remain 3. Reviewed the current changes to the UI shell, properties, editor transaction boundary, design geometry, asset import, persistence, version validation and manuals. Parser, compiler, scheduler, evaluation order, runtime limits, runtime geometry and executable schemas retain their existing implementation. Full runtime source comparison permits exactly the product appVersion literal in addition to the recorded identifier map. The complete 16-phase gate and all 28 D09 obligations remain applicable; this source review does not convert pending execution into PASS.

Duplication builds and validates a candidate before replacing the current project. Component, costume and local-data IDs are allocated from a shared reserved set; local arrays and definitions are deep copies. Names are unique and bounded to 80 Unicode code points. Immutable asset bytes are shared and normal reference compaction preserves an asset used by the copy after deleting the original. No source/script is added for the copy. The existing editor flush, lock, history, render and editor-load paths are reused. Offset placement accounts for scale and rotation, including a 500-pixel image scaled to the stage height. Component, file and resource limits remain enforced by the normal serializer and import paths.

The drawing dialog owns a 500 by 500 transparent canvas and at most 31 pixel snapshots (30 undo operations). Pointer coordinates are mapped to canvas pixels at the actual viewport scale. Pointer capture, cancellation, lost capture and window blur terminate or revert the current gesture. Global keyboard dispatch is guarded by the open drawing dialog, so undo/redo still work after a focused undo button becomes disabled. The existing shared modal focus trap handles Tab; background controls are inert. Drawing operations do not write project history. A successful commit runs the normal transactional PNG import once; an error leaves the drawing intact. The alpha bounding box removes only exterior transparent rows/columns, preserves holes, and pads a sub-8-pixel dimension without resampling. Oversized drawings use uniform scalePercent around their center. Closing clears the temporary canvas/history; no new persistence or network path exists. Uncommitted ink participates in beforeunload protection.

The 1.0.0 importer verifies original-version metadata, Markdown, embedded bytes, hashes, decoding, dimensions, references and limits before promoting appVersion. Unsupported versions and malformed data retain rejection and current-state protection. The serializer emits the current version. Generated HTML uses its own embedded runtime and retains offline execution. Fixed 1.0.0 and 0.8 products, suites, manifests and provenance are unchanged. Three current frozen-origin suite literals are explicitly mapped in `release-101-contract.mjs`; full assertion-source equality still applies. Supplementary version-sensitive expectations and malformed-version inputs are updated to 1.0.1, with no removed test IDs or thresholds.

Fifteen additional real-browser cases cover all component types, identity/local-data independence, immutable assets, limits/history, both editor modes, six drawing tools, swatches and free color, antialiasing/eraser/fill, bounded drawing history, canceled gestures, focus, crop/padding, stage fitting, discard, failed commit retention, save/reload/export, runtime lock, narrow viewport and trusted touch input, and 1.0.0 compatibility/invalid inputs. Their reports are validated independently for exact IDs, snapshot, environment, PASS and absence of page errors/network. Negative tests reject ten malformed reports and unauthorized runtime/version changes. The local gate includes both additions and continues after failures; Actions uses that same gate with unchanged workflow triggers, five browser groups and aggregate requirements.

Desktop screenshot review found inherited fixed dialog height and button colors masking the new palette. Scoped drawing-dialog sizing and swatch styles now keep the toolbar and 500-pixel canvas together on a sufficiently tall screen, with scrolling on smaller screens. The regression checks assert desktop fit and actual swatch color. User documentation states duplication without code, drawing tools, transparent crop/padding, normal image placement and manual saving. The advanced guide explicitly distinguishes product 1.0.1 from language/runtime 1.0.0.

The input binding is renewed after this review. Final evidence must be regenerated for the committed HEAD and checked again on GitHub Actions before the Draft PR is reported as verified. Local tests use installed Chrome because the existing pinned Windows Chromium executable fails at process spawn; Actions retains its pinned Playwright/Chromium environment and mandatory startup probe. Headless touch emulation does not establish physical-device, native IME, user-study or learning-effect results. Ready, merge and release publication are outside this task's completion boundary.

## Verified 1.0.1 audit checkpoint

The user explicitly requested moving the fixed audit checkpoint to 1.0.1 and adding the correction on the existing branch. The exact source is 62359484a646651dd81806ef97a6cfef87f85d4b, already successful in all eight jobs of push run 36017186869/1 and PR run 36018691411/1. The archive retains the push evidence: all eight original ZIP digests, 221 executed input hashes against exact Git blobs, all seven sealed file sets and 2,485 file hashes were verified during capture. Selected original reports, bundle manifests and aggregate remain unchanged. Thirty-four source paths have self-contained commit/tree/blob proofs. The checkpoint manifest is pinned by SHA-256. Ready, merge and release authorization are not claimed; checkpointApproved is true and releaseComplete remains false.

Reviewed all active references in the quality policy, core and language runners, runtime comparison, browser baseline copies and independent validation, inventory generator, workflow/preflight and governing documents. They now resolve the immutable 1.0.1 source. Core assertions match both the recorded historical transformation and the complete 1.0.1 frozen source. Language/editor assertions, 257 capability mappings, 27 browser tasks / 416 expected tuples and 605/38/9 language/editor/GUI IDs remain exact. Runtime equality no longer requires the historical metadata adapter. Product code is unchanged by this checkpoint correction.
