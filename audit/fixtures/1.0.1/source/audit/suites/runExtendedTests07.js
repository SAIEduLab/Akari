async function runExtendedTests07() {
    ...
    await test('HINT generated names stay valid through collision boundary', () => {
      ...
      const each = A.hintCandidates(p, 'stage').find((x) => x.id === 'each:作品の名前一覧');
      ...
      const args = [...chain('結果'), '結果2'],
        q = make();
      let hints = A.hintCandidates(q, 'stage', {
          source: '',
          start: 0,
          end: 0,
      ...
      const first = scalar.text + '\n',
        at = first.length;
      hints = A.hintCandidates(q, 'stage', {
        source: first,
        start: at,
        end: at,
        definitionKind: 'action',
    ...
    await test('HINT event-specific values and target restrictions', () => {
      for (const [event, sensor] of [
        ...
        ok(
          A.hintCandidates(make(), 'stage', { event }).some(
            (x) => x.text === sensor && x.mode === 'expression',
          ),
        );
        ok(!A.hintCandidates(make(), 'stage', { event: 'start' }).some((x) => x.text === sensor));
        ok(
          !A.hintCandidates(make(), 'stage', { event, definitionKind: 'action' }).some(
            (x) => x.text === sensor,
          ),
        );
      }
      ok(A.hintCandidates(make(), 'stage').find((x) => x.id === 'MotionCommand:MOVE').reason);
    });
    ...
    await test('HINT source insertion preserves exact selection surroundings', () => {
      ...
      const a = A.insertSource(source, 3, 4, '7', 'expression');
      eq(a.source, '点数に7を足す');
      const b = A.insertSource('  \nnext', 2, 2, '3回くり返す\n  x\n' + END);
      eq(b.source, '  3回くり返す\n    x\n  ' + END + '\nnext');
      eq(A.insertSource('old\nnext', 0, 3, 'new').source, 'new\nnext');
    });
    await test('HINT mid-line statement insertion refused', () =>
      reject(() => A.insertSource('abcdef', 2, 3, 'x'), 'P201'));
    ...
    await test('CLONE collision transform ordering and clone data independence', () => {
      ...
      clone.vars.set('HP', 9);
      eq(a.vars.get('HP'), 3);
      ok(h.r.touching(clone.runtimeId, STANDARD_AKARI_MASCOT.name));
      clone.visible = false;
      ok(!h.r.touching(clone.runtimeId, STANDARD_AKARI_MASCOT.name));
      a.direction = 90;
      a.scalePercent = 200;
