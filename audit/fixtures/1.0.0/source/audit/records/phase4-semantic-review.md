...
`phase3-runtime-contract.mjs` reconstructs the complete HA runtime after reversing only the explicitly allowed version and diagnostic-location differences. `language-boundaries.mjs` also compares API keys, all 153 schema IDs, 74 catalog entries and 38 limits. The A-only `product-diff.mjs` is not used to claim invariance after language/editing changes.
...
## S09 — Save, restore, autosave and drafts

Reviewed `serializeProject`, `parseProjectFile`, `validateSerializedProjectStructure`, asset-reference/digest/decode validation, `packExecutable`/restore, editor candidate preparation, `writeAutosave`, `readAutosave`, recovery epoch/dirty/pending gates and explicit callable registration.

Project source, including invalid unfinished source, is persisted independently of its diagnostics. A callable draft is kept separately until explicit registration; autosave stores draft and committed project separately. Imports validate structure, IDs, limits, canonical Markdown body, digest and real material decoding before installation. Body/recovery-data disagreement raises F512. Failed imports/recovery cannot replace the current project, pending source or common history. Recovery checks again after asynchronous decoding to avoid installing stale data over later edits. The IndexedDB transaction deadline remains five seconds; the fallback refuses unsupported material-bearing records rather than discarding assets.

Format-3 markers and storage namespaces are shared definitions. Product version and data-format numbers have distinct purposes. The exact current structure is validated without an enumerated old-version/old-marker rejection branch, conversion or migration. No guarantee of old-file compatibility or certain old-file rejection is inferred. Shared accepted grammar remains usable regardless of origin.
