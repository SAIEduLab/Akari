...
function runLanguageForms09(api, manifest, golden, legacy, schemaIds, inlineCases, inlineNegative) {
  ...
  for (const c of manifest.cases) {
    ...
    test(id,()=>{
      ...
      ok(api.astEquivalent09(ast,c.canonicalAst),'canonical AST mismatch');
      eq(ast.source,c.source);
      const formatted=api.formatScript(ast);
      eq(formatted,c.canonicalFormatted);
      ok(api.astEquivalent09(ast,parsed(formatted)),'format reparse');
      eq(api.formatScript(parsed(formatted)),formatted);
      eq(exec(c.source),golden[c.id+'/'+c.key]);
    });
  }
  test('A09-ORDER-009',()=>{for(const c of legacy)ok(api.astEquivalent09(parsed(c.source),c.ast),c.source);});
  test('A09-ORDER-012',()=>eq(api.BLOCK_SCHEMAS09.map(s=>s.id),schemaIds));
  test('A09-ORDER-006',()=>{
    ...
    for(const initialValue of [0,'文字',false]) {
      ...
      ok(api.astEquivalent09(ast,changed.ast));
    }
    const p=api.makeDefaultProject();
  ...
  test('A09-ORDER-007',()=>{
    ...
    ok(api.astEquivalent09(parsed('（2＋3）を作品の点数に加える'),parsed('作品の点数に（2＋3）を足す')));
    ok(api.astEquivalent09(parsed('名前一覧の1番目を点数に加える'),parsed('点数に名前一覧の1番目を足す')));
  ...
  for (const c of inlineCases) test(c.id,()=>{
    ok(api.astEquivalent09(parsed(c.source),parsed(c.canonical)),'inline AST');
  ...
  test('A09-ORDER-004',()=>{
    ...
    for(const sensor of ['「a」キーが押されている','「a」キーが押されていない','マウスが押されている','マウスが押されていない']) {
      for(const source of [`${sensor}が真と等しいという`,`真が${sensor}と等しいという`]) {
        ...
        ok(api.astEquivalent09(parsed(source),parsed(canonical)),'sensor compound role boundary');
        eq(exec(source),exec(canonical));
      }
    }
  });
