...
function runEditorSurface10(api, forms) {
  ...
  const setup = source => {
    ...
    return api.createEditorSession09('script:stage:start',source,{targetId:'stage',event:'start'},p,4);
  };
  const change = (s,id,value,key='value') => api.prepareBlockEdit09(s,{type:'field',id,key,value});
  ...
  for(const [key,line,path,value,expected] of edits) test('A10-EDIT/'+key,()=>{
    ...
    ok(api.astEquivalent09(result.syntaxAst,api.blockDecode(result.blockView)));
  });
  ...
  test('A10-EDIT/all-forms-noop',()=>{
    for(const c of forms) {
      ...
      const result=api.prepareBlockEdit09(s,{type:'noop'});
  ...
  test('A10-EDIT/inline-expand',()=>{
    ...
    const result=api.prepareBlockEdit09(s,{type:'insert',parentId:branch.id,body:'thenBody',index:1,node:api.createBlock09('NoOperation')});
    eq(result.source,'※ 前  \nもし 真なら、次のことをする。\n  点数に3を足す。  ※ 本文  \n  何もしない。\n\n「😀」  という ※ 後  \n');
  ...
  test('A10-EDIT/insert-remove-move',()=>{
    ...
    eq(api.prepareBlockEdit09(s,{type:'move',id:b[1].id,parentId:s.blockView.id,body:'body',index:0}).source,
      '2を点数に加える ※ 後  \n1を点数に足す\n3秒間待つ');
    eq(api.prepareBlockEdit09(s,{type:'remove',id:b[1].id}).source,'1を点数に足す\n\n3秒間待つ');
  });
  test('A10-EDIT/atomic-stale-ime-limit',()=>{
    ...
    const reject=options=>{const before=JSON.stringify(s);let failed=false;try{api.prepareBlockEdit09(s,op,options);}catch{failed=true;}ok(failed);eq(JSON.stringify(s),before);};
  ...
  test('A10-EDIT/batch-role-offsets',()=>{
    ...
    const result=api.prepareBlockEdit09(s,{type:'batch',operations:[
  ...
  test('A10-EDIT/predicate-in-expression-slot',()=>{
    ...
    for(const [schema,expression] of [
      ...
      const result=api.prepareBlockEdit09(s,{type:'replace',id:binary.inputs.left.id,node:api.createBlock09(schema)});
  ...
  test('A10-EDIT/condition-replacement-keeps-body',()=>{
    ...
    const result=api.prepareBlockEdit09(s,{type:'replace',id:n.id,node:api.createBlock09('CompareExpression:GT')});
  ...
  test('A10-EDIT/inline-finite-statement-composition',()=>{
    ...
    for(const c of forms.filter(c=>!c.source.includes('\n'))) {
      ...
      ok(actual.ast&&expected.ast,c.source);ok(api.astEquivalent09(actual.ast,expected.ast),c.source);checked++;
    }
