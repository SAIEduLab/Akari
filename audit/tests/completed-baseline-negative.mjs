...
assert.ok(gateSteps('browser','output').some(s=>s[0]==='fixed102'&&s[1]==='audit/run-fixed-baseline.mjs'));
assert.ok(gateSteps('browser','output').some(s=>s[0]==='fixed08'&&s[1]==='audit/run-headless-selftest.mjs'));
const report=JSON.parse(fs.readFileSync(completedFixture+'/evidence/akari-selftest-evidence-36235604048-1/current-selftest.json'));
